...
u8 updata_fontx(u16 x,u16 y,u8 size,u8 *fxpath,u8 fx)
{
 	...
 	if(rval==0)	 
	{
		...
		while(res==FR_OK)//死循环执行
		{
			...
			writeW25x(tempbuf,offx+flashaddr,4096);		//从0开始写入4096个数据  
	  		offx+=bread;	  
...
{	
	...
	{  
		...
		for(i=0;i<FONTSECSIZE;i++)	//先擦除字库区域,提高写入速度
		{
			...
			readW25x((u8*)buf,((FONTINFOADDR/4096)+i)*4096,4096);//读出整个扇区的内容
			for(j=0;j<1024;j++)//校验数据
			{
				if(buf[j]!=0XFFFFFFFF)break;//需要擦除  	  
			}
			if(j!=1024)eraseW25xSector((FONTINFOADDR/4096)+i);	//需要擦除的扇区
		}
		myfree(SRAMIN,buf);
		...
		//全部更新好了
		ftinfo.fontok=0XAA;
		writeW25x((u8*)&ftinfo,FONTINFOADDR,sizeof(ftinfo));	//保存字库信息
	}
...
{		
	...
	{
		t++;
		readW25x((u8*)&ftinfo,FONTINFOADDR,sizeof(ftinfo));//读出ftinfo结构体数据
