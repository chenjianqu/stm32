...
/*
__align 关键字指示编译器在 n 字节边界上对齐变量。__align 是一个存储类修饰符。它不影响函数的类型。
编译器自行决定code的再flash/ram中的地址，我们在编译后才能知道具体地址在哪里。在有些场合我们需要指定变量/常量的地址，就要用到__attribute__((at(0xXXXXXXXX))) 
__attribute__((at(0x0803f800)))就是指定了code定义的位置为0x0803f800。
*/
